...
PDF = r"data\Boletin_Chilehuevos_Ago_2025.pdf"  # ajusta si renombraste
...
def clean_number_series(x):
    # limpia miles, cambia coma decimal, quita %, convierte a num (NaN si no se puede)
    s = (x.astype(str)
...
# localizar cabecera (fila que contiene "Mes")
hdr_i = 0
...
# comprimir pares de columnas (encabezado en 2 líneas)
cols = []
i=0
while i < len(hdr):
    t1 = str(hdr[i]).strip()
...
# % puede venir sin palabra clara; buscamos cualquier header con símbolo %
c_pct  = None
for c in rd.columns:
...
if c_imp:
    s_imp = pd.DataFrame({"date": date})
    s_imp["value"] = clean_number_series(rd[c_imp])
    out["mercado_importacion.csv"] = s_imp.dropna().sort_values("date")

if c_tot:
    s_tot = pd.DataFrame({"date": date})
    s_tot["value"] = clean_number_series(rd[c_tot])
    out["mercado_total.csv"] = s_tot.dropna().sort_values("date")

# % importados directo (si existe la col y tiene números válidos)
if c_pct:
    s_pct = pd.DataFrame({"date": date})
    s_pct["value"] = clean_number_series(rd[c_pct])
    s_pct = s_pct.dropna().sort_values("date")
    # si quedó vacía o casi vacía, la reconstruimos más abajo
    if not s_pct.empty and s_pct["value"].notna().sum() >= 6:
        out["mercado_pct_importados.csv"] = s_pct

# reconstrucción si no quedó usable la columna %
if "mercado_pct_importados.csv" not in out and {"mercado_importacion.csv","mercado_total.csv"} <= set(out.keys()):
    ...
    m = m[(m["value_tot"].notna()) & (m["value_tot"] != 0)]
    m["value"] = (m["value_imp"]/m["value_tot"])*100.0
    out["mercado_pct_importados.csv"] = m[["date","value"]].sort_values("date")
